...
PySceneDetect's detectors are benchmarked for accuracy and speed against public
...
hard-cut scores are expected, and it is included for completeness only.

## Parameter sweeps

Beyond the defaults, a full grid sweep over each detector's key parameters shows how
accuracy responds to tuning, and how much the optimum differs per dataset:
...
Dots mark each dataset's optimum within the shown parameter slice. The key takeaway:
per-dataset optima diverge - long-form broadcast content (BBC) generally prefers lower
thresholds than short web clips (ClipShots) - so the defaults aim for a robust
middle ground rather than any single corpus's peak.

How much accuracy is on the table? The chart below compares three levels for each detector:
the v0.7 default (tick), the best *single* parameter set across all datasets (bar - what a
better default could ship), and each dataset's own optimum (dot - the ceiling when tuning for
...
Two things stand out. HistogramDetector's default is far from its optimum everywhere
(most dramatically on ClipShots: 20 to 48). And for ContentDetector / AdaptiveDetector on
BBC, the v0.7 default actually *beats* the cross-dataset tuned setting - tuning toward
short web clips costs accuracy on long-form broadcast, which is why defaults favor a robust
middle ground.

Scored by mean hard-cut F1 at 1-frame tolerance across all three datasets, the best single
parameter set per detector compares to the v0.7 defaults as follows:
...
A review of the default values in light of these results is in progress; HistogramDetector's
default in particular scores well below its swept optimum (54.3 vs 66.3 mean F1).

## Reproducing

See [`benchmark/README.md`](https://github.com/Breakthrough/PySceneDetect/blob/main/benchmark/README.md)
for dataset download instructions and usage. In short:
