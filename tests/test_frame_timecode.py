...
def test_get_frames():
    ...
    assert FrameTimecode(timecode=1000000000.0, fps=29.97).get_frames(), int(
        1000000000.0 * 29.97
    )
...
def test_get_seconds():
    ...
    assert FrameTimecode(timecode=1000, fps=60.0).get_seconds(), pytest.approx(
        1000 / 60.0
    )
    ...
    assert FrameTimecode(timecode="00:00:02.0000", fps=1).get_seconds(), pytest.approx(
        2.0
    )
    assert FrameTimecode(timecode="00:00:00.5", fps=10).get_seconds(), pytest.approx(
        0.5
    )
    assert FrameTimecode(timecode="00:00:01", fps=10).get_seconds(), pytest.approx(1.0)
    assert FrameTimecode(timecode="00:01:00.000", fps=1).get_seconds(), pytest.approx(
        60.0
    )
...
def test_get_timecode():
    ...
    assert (
        FrameTimecode(timecode="00:00:02.0000", fps=1).get_timecode() == "00:00:02.000"
    )
    assert FrameTimecode(timecode="00:00:00.5", fps=10).get_timecode() == "00:00:00.500"
    assert (
        FrameTimecode(timecode="00:00:01.501", fps=10).get_timecode() == "00:00:01.500"
    )
    assert (
        FrameTimecode(timecode="00:01:00.000", fps=1).get_timecode() == "00:01:00.000"
    )
...
@pytest.mark.parametrize(
    "frame_num,fps", [(1, 1), (61, 14), (29, 25), (126, 24000 / 1001.0)]
)
...
def test_precision():
    ...
    assert (
        FrameTimecode(110, fps).get_timecode(precision=2, use_rounding=True)
        == "00:00:00.11"
    )
    assert (
        FrameTimecode(110, fps).get_timecode(precision=2, use_rounding=False)
        == "00:00:00.11"
    )
    assert (
        FrameTimecode(110, fps).get_timecode(precision=1, use_rounding=True)
        == "00:00:00.1"
    )
    assert (
        FrameTimecode(110, fps).get_timecode(precision=1, use_rounding=False)
        == "00:00:00.1"
    )
    assert (
        FrameTimecode(110, fps).get_timecode(precision=0, use_rounding=True)
        == "00:00:00"
    )
    assert (
        FrameTimecode(110, fps).get_timecode(precision=0, use_rounding=False)
        == "00:00:00"
    )

    assert (
        FrameTimecode(990, fps).get_timecode(precision=2, use_rounding=True)
        == "00:00:00.99"
    )
    assert (
        FrameTimecode(990, fps).get_timecode(precision=2, use_rounding=False)
        == "00:00:00.99"
    )
    assert (
        FrameTimecode(990, fps).get_timecode(precision=1, use_rounding=True)
        == "00:00:01.0"
    )
    assert (
        FrameTimecode(990, fps).get_timecode(precision=1, use_rounding=False)
        == "00:00:00.9"
    )
    assert (
        FrameTimecode(990, fps).get_timecode(precision=0, use_rounding=True)
        == "00:00:01"
    )
    assert (
        FrameTimecode(990, fps).get_timecode(precision=0, use_rounding=False)
        == "00:00:00"
    )
