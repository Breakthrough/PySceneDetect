...
def test_cli_detector(detector_command: str):
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} time {TIME} {DETECTOR}", DETECTOR=detector_command
        )
        == 0
    )
...
def test_cli_split_video_ffmpeg(tmp_path: Path):
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -s {STATS} time {TIME} {DETECTOR} split-video",
            output_dir=tmp_path,
        )
        == 0
    )
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -s {STATS} time {TIME} {DETECTOR} split-video -c",
            output_dir=tmp_path,
        )
        == 0
    )
...
@pytest.mark.skipif(
    condition=not is_mkvmerge_available(), reason="mkvmerge is not available"
)
def test_cli_split_video_mkvmerge(tmp_path: Path):
    """Test `split-video` command using mkvmerge."""
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -s {STATS} time {TIME} {DETECTOR} split-video -m",
            output_dir=tmp_path,
        )
        == 0
    )
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -s {STATS} time {TIME} {DETECTOR} split-video -m -c",
            output_dir=tmp_path,
        )
        == 0
    )
...
def test_cli_save_images(tmp_path: Path):
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -s {STATS} time {TIME} {DETECTOR} save-images",
            output_dir=tmp_path,
        )
        == 0
    )
...
def test_cli_export_html(tmp_path: Path):
    ...
    assert (
        invoke_scenedetect(
            base_command, COMMAND="save-images export-html", output_dir=tmp_path
        )
        == 0
    )
    assert (
        invoke_scenedetect(
            base_command, COMMAND="export-html --no-images", output_dir=tmp_path
        )
        == 0
    )
...
def test_cli_backend(backend_type: str):
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -b {BACKEND} time {TIME} {DETECTOR}", BACKEND=backend_type
        )
        == 0
    )
...
def test_cli_backend_unsupported():
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} -b {BACKEND} {DETECTOR}", BACKEND="unknown_backend_type"
        )
        != 0
    )
...
def test_cli_load_scenes():
    ...
    assert (
        invoke_scenedetect(
            "-i {VIDEO} time {TIME} load-scenes -i {VIDEO_NAME}-Scenes.csv"
        )
        == 0
    )
