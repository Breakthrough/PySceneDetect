...
def format_option(
    command: click.Command, opt: click.Option, flags: ty.List[str]
) -> StrGenerator:
    ...
    help = (
        ...
        if command.name in OPTION_HELP_OVERRIDES
        and opt.name in OPTION_HELP_OVERRIDES[command.name]
        else opt.help.strip()
    )
...
def generate_command_help(
    ...
    help = help.replace("\b\n", "")
    help = help.format(
        scenedetect="scenedetect", scenedetect_with_video="scenedetect -i video.mp4"
    )
...
def generate_subcommands(ctx: click.Context, commands: ty.List[str]) -> StrGenerator:
    ...
    for detector in detectors:
        yield from generate_command_help(
            ctx, ctx.command.get_command(ctx, detector), ctx.info_name
        )
    ...
    for command in output_commands:
        yield from generate_command_help(
            ctx, ctx.command.get_command(ctx, command), ctx.info_name
        )
