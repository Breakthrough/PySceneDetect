...
**The** ``load-scenes`` **command** is used to load scenes from CSV generated by `list-scenes`.
Only the start time column is required.


Example
-----------------------------------------------------------------------

To manually edit the cut points when generating output (e.g. using `split-video`), first generate a `video-Scenes.csv` file using `list-scenes`:

    ``scenedetect --input video.mp4 detect-content list-scenes``

Manually edit the created `video-Scenes.csv` file, and then re-run with `load-scenes` instead of the specified detector followed by other output commands:
