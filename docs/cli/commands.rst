...
**The** ``load-scenes`` **command** load scenes from CSV instead of detecting. It can be used with CSV files generated by `list-scenes <list-scenes Command>`. Scenes are loaded using the specified column as cut locations (frame number or timecode).
