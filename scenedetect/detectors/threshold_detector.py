    ...
    def process_frame(self, frame_num: int, frame_img: numpy.ndarray) -> List[int]:
        ...
        if self.processed_frame:
            if self.last_fade["type"] == "in" and (
                (
                    self.method == ThresholdDetector.Method.FLOOR
                    and frame_avg < self.threshold
                )
                or (
                    self.method == ThresholdDetector.Method.CEILING
                    and frame_avg >= self.threshold
                )
            ):
                ...
                if (frame_num - self.last_scene_cut) >= self.min_scene_len:
                    ...
                    f_split = int(
                        (frame_num + f_out + int(self.fade_bias * (frame_num - f_out)))
                        / 2
                    )
