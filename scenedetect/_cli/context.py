...
def check_split_video_requirements(use_mkvmerge: bool) -> None:
    ...
    if (use_mkvmerge and not is_mkvmerge_available()) or not is_ffmpeg_available():
        ...
        if not use_mkvmerge and is_mkvmerge_available():
            error_strs += [
                "You can specify mkvmerge (-m) to use mkvmerge for splitting."
            ]
...
class CliContext:
    ...
    def __init__(self):
        ...
        self.default_detector: Tuple[Type[SceneDetector], Dict[str, Any]] = (
        ...
        self.cut_format: TimecodeFormat = (
            TimecodeFormat.TIMECODE
        )  # [list-scenes] cut-format
    ...
    def handle_options(
        ...
        try:
            ...
            if config and not init_failure:
                ...
                if verbosity is None and not self.config.is_default(
                    "global", "verbosity"
                ):
        ...
        self._open_video_stream(
            ...
            framerate=framerate,
            backend=self.config.get_value(
                "global", "backend", backend, ignore_default=True
            ),
        )

        self.output_dir = (
            output if output else self.config.get_value("global", "output")
        )
        if self.output_dir:
        ...
        if default_detector == "detect-adaptive":
            self.default_detector = (
                AdaptiveDetector,
                self.get_detect_adaptive_params(),
            )
            ...
            self.default_detector = (
                ThresholdDetector,
                self.get_detect_threshold_params(),
            )
        else:
            raise click.BadParameter(
                "Unknown detector type!", param_hint="default-detector"
            )
    ...
    def get_detect_content_params(
        ...
        if self.drop_short_scenes:
            ...
            if min_scene_len is None:
                if self.config.is_default("detect-content", "min-scene-len"):
                    ...
                    min_scene_len = self.config.get_value(
                        "detect-content", "min-scene-len"
                    )
            min_scene_len = parse_timecode(
                min_scene_len, self.video_stream.frame_rate
            ).frame_num
        ...
        return {
            ...
            "kernel_size": self.config.get_value(
                "detect-content", "kernel-size", kernel_size
            ),
            "luma_only": luma_only
            or self.config.get_value("detect-content", "luma-only"),
            "min_scene_len": min_scene_len,
            "threshold": self.config.get_value(
                "detect-content", "threshold", threshold
            ),
            "filter_mode": FlashFilter.Mode[
                self.config.get_value(
                    "detect-content", "filter-mode", filter_mode
                ).upper()
            ],
        }
    ...
    def get_detect_adaptive_params(
        ...
        if min_delta_hsv is not None:
            logger.error(
                "-d/--min-delta-hsv is deprecated, use -c/--min-content-val instead."
            )
        ...
        if weights is not None:
            try:
                ...
                raise click.BadParameter(str(ex), param_hint="weights")
        return {
            "adaptive_threshold": self.config.get_value(
                "detect-adaptive", "threshold", threshold
            ),
            "weights": self.config.get_value("detect-adaptive", "weights", weights),
            "kernel_size": self.config.get_value(
                "detect-adaptive", "kernel-size", kernel_size
            ),
            "luma_only": luma_only
            or self.config.get_value("detect-adaptive", "luma-only"),
            "min_content_val": self.config.get_value(
                "detect-adaptive", "min-content-val", min_content_val
            ),
            "min_scene_len": min_scene_len,
            "window_width": self.config.get_value(
                "detect-adaptive", "frame-window", frame_window
            ),
        }
    ...
    def get_detect_threshold_params(
        ...
        if self.drop_short_scenes:
            ...
            if min_scene_len is None:
                if self.config.is_default("detect-threshold", "min-scene-len"):
                    ...
                    min_scene_len = self.config.get_value(
                        "detect-threshold", "min-scene-len"
                    )
            min_scene_len = parse_timecode(
                min_scene_len, self.video_stream.frame_rate
            ).frame_num
        ...
        return {
            ...
            "fade_bias": self.config.get_value(
                "detect-threshold", "fade-bias", fade_bias
            ),
            "min_scene_len": min_scene_len,
            "threshold": self.config.get_value(
                "detect-threshold", "threshold", threshold
            ),
        }
    ...
    def handle_load_scenes(self, input: AnyStr, start_col_name: Optional[str]):
        ...
        if self.added_detector:
            raise click.ClickException(
                "The load-scenes command cannot be used with detectors."
            )
        if self.load_scenes_input:
            raise click.ClickException(
                "The load-scenes command must only be specified once."
            )
        input = os.path.abspath(input)
        if not os.path.exists(input):
            raise click.BadParameter(
                f"Could not load scenes, file does not exist: {input}",
                param_hint="-i/--input",
            )
    ...
    def handle_export_html(
        ...
        self.html_name_format = self.config.get_value(
            "export-html", "filename", filename
        )
        self.image_width = self.config.get_value(
            "export-html", "image-width", image_width
        )
        self.image_height = self.config.get_value(
            "export-html", "image-height", image_height
        )
    ...
    def handle_split_video(
        ...
        if contains_sequence_or_url(self.video_stream.path):
            error_str = (
                "The split-video command is incompatible with image sequences/URLs."
            )
            raise click.BadParameter(error_str, param_hint="split-video")
        ...
        self.split_dir = self.config.get_value(
            "split-video", "output", output, ignore_default=True
        )
        if self.split_dir is not None:
            logger.info("Video output path set:  \n%s", self.split_dir)
        self.split_name_format = self.config.get_value(
            "split-video", "filename", filename
        )
        ...
        if mkvmerge or copy:
            ...
            if args:
                raise click.BadParameter(
                    "args (-a) cannot be used with %s" % (command),
                    param_hint="split-video",
                )
            if rate_factor:
                raise click.BadParameter(
            ...
            if preset:
                raise click.BadParameter(
                    "preset (-p) cannot be used with %s" % (command),
                    param_hint="split-video",
                )
    ...
    def handle_save_images(
        ...
        if (
            not self.image_extension in valid_params
            or valid_params[self.image_extension] is None
        ):
            error_strs = [
        ...
        self.image_dir = self.config.get_value(
            "save-images", "output", output, ignore_default=True
        )

        self.image_name_format = self.config.get_value(
            "save-images", "filename", filename
        )
        self.num_images = self.config.get_value("save-images", "num-images", num_images)
        self.frame_margin = self.config.get_value(
            "save-images", "frame-margin", frame_margin
        )
    ...
    def handle_time(self, start, duration, end):
        ...
        self.start_time = parse_timecode(
            start, self.video_stream.frame_rate, correct_pts=True
        )
    ...
    def _open_video_stream(
        self, input_path: AnyStr, framerate: Optional[float], backend: Optional[str]
    ):
        ...
        if framerate is not None and framerate < MAX_FPS_DELTA:
            raise click.BadParameter(
                "Invalid framerate specified!", param_hint="-f/--framerate"
            )
        try:
            if backend is None:
                ...
                if not backend in AVAILABLE_BACKENDS:
                    raise click.BadParameter(
                        "Specified backend %s is not available on this system!"
                        % backend,
            ...
            if backend == "pyav":
                self.video_stream = open_video(
                    ...
                    framerate=framerate,
                    backend=backend,
                    threading_mode=self.config.get_value(
                        "backend-pyav", "threading-mode"
                    ),
                    suppress_output=self.config.get_value(
                        "backend-pyav", "suppress-output"
                    ),
                )
            elif backend == "opencv":
                self.video_stream = open_video(
            ...
            logger.debug(
                "Video opened using backend %s", type(self.video_stream).__name__
            )
