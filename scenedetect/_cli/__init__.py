...
DETECT_KOALA_HELP = """Find fast cuts using the Koala-36M transition model (experimental).
...
Examples:
    ...
    {scenedetect_with_video} detect-koala --threshold -0.5 --adaptive
"""
...
@click.option(
    ...
    "-t",
    metavar="VAL",
    type=_click_range("detect-koala", "threshold"),
    default=None,
    help="Frames scoring below this value are flagged as part of a transition. Lower values "
    "are less sensitive to changes.{}".format(
        USER_CONFIG.get_help_string("detect-koala", "threshold")
    ),
)
@click.option(
    "--adaptive/--no-adaptive",
    default=None,
    help="Also flag frames whose smoothed score drops far below the mean of the preceding "
    "frames, as in the reference implementation. Off by default as it reduced accuracy on "
    "benchmarks.{}".format(USER_CONFIG.get_help_string("detect-koala", "adaptive")),
)
@click.option(
    "--min-scene-len",
    "-m",
    ...
    % (
        ""
        if USER_CONFIG.is_default("detect-koala", "min-scene-len")
        else USER_CONFIG.get_help_string("detect-koala", "min-scene-len")
    ),
)
@click.pass_context
def detect_koala_command(
    ...
    ctx = ctx.obj
    assert isinstance(ctx, CliContext)
    detector_args = ctx.get_detect_koala_params(
        threshold=threshold, adaptive=adaptive, min_scene_len=min_scene_len
    )
    ctx.add_detector(KoalaDetector, detector_args)
