    ...
    def format_help_text(
        ...
        if self.help:
            ...
            formatted_help = self.help.format(
                scenedetect=base_command,
                scenedetect_with_video="%s -i video.mp4" % base_command,
            )
...
@click.option(
    ...
    help="Path to config file. If unset, tries to load config from %s"
    % (CONFIG_FILE_PATH),
)
@click.option(
    "--stats",
...
@click.option(
    ...
    % (
        ...
        USER_CONFIG.get_help_string("global", "backend"),
    ),
)
@click.option(
    "--downscale",
...
@click.option(
    ...
    metavar="NUM",
    type=click.IntRange(
        CONFIG_MAP["detect-hist"]["bins"].min_val,
...
@click.option(
    ...
    metavar="SIZE",
    type=click.IntRange(
        CONFIG_MAP["detect-hash"]["size"].min_val,
...
@click.option(
    ...
    metavar="FRAC",
    type=click.IntRange(
        CONFIG_MAP["detect-hash"]["lowpass"].min_val,
...
def load_scenes_command(
    ctx: click.Context, input: Optional[str], start_col_name: Optional[str]
):
...
@click.option(
    ...
    is_flag=True,
    flag_value=True,
    help="Suppress printing scene list.%s"
