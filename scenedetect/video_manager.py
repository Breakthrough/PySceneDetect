...
def open_captures(
    ...
    try:
        ...
        closed_caps = [
            video_names[i] for i, cap in enumerate(cap_list) if not cap.isOpened()
        ]
        if closed_caps:
        ...
        cap_frame_sizes = [
            (
                math.trunc(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
...
def validate_capture_framerate(
    ...
    if framerate is not None:
        if isinstance(framerate, float):
            ...
            raise TypeError(
                "Expected float for framerate, got %s." % type(framerate).__name__
            )
    else:
...
class VideoManager(VideoStream):
    ...
    def __init__(
        ...
        if self._logger is not None:
            self._logger.info(
                ...
                len(self._cap_list),
                "s" if len(self._cap_list) > 1 else "",
                self.get_framerate(),
                *self.get_framesize(),
            )
        self._started = False
        self._frame_length = self.get_base_timecode() + get_num_frames(self._cap_list)
        self._first_cap_len = self.get_base_timecode() + get_num_frames(
            [self._cap_list[0]]
        )
    ...
    def set_duration(
        ...
        if (
            (duration is not None and not duration.equal_framerate(self._cap_framerate))
            or (
    ...
    def seek(
        self, timecode: FrameTimecode = None, target: FrameTimecode = None
    ) -> bool:
